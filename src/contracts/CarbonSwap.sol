...
contract CarbonSwap is IERC20, Ownable {
	using SafeMath for uint256;

	IERC20 public usdc;
	IERC20 public ccoin;

	constructor(address _usdc, address _ccoin) {
		usdc = IERC20(_usdc);
		ccoin = IERC20(_ccoin);
	}
	...
	function swap(uint256 _amountusdc) public {
		...
		ccoin.transfer(_msgSender(), _amountccoin);
	}
	...
	function withdrawbalance() public onlyOwner {
		uint256 balance = ccoin.balanceOf(address(this));
		ccoin.transfer(owner(), balance);
	}
